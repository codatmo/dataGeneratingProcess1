SirtdVaryBeta <- function(runName = runName,
  ...
  for (day in 1:nDays) {
    ...
    for (per in 1:nPop) {
      ...
      if (dayState[per] == 'i') {
        tweets[per] = rpois(1, tweetRateInfected)
    ...
    for (per in 1:nPop) {
      if (dayState[per] == 'i') {
        for (otherPer in sample(1:nPop, rpois(n = 1, lambda = betaDailyInfectionRates[day]))) {
          if (dayState[otherPer] == 's') {  #subtle but will reduce infs if there are not lots of s
