...
transformed parameters{
  ...
  vector[n_days+1] state_S;
  vector[n_days+1] state_I;
  vector[n_days+1] state_R;
  vector[n_days+1] state_D;
  vector[n_days] daily_infections;
  vector[n_days] daily_deaths;
  vector[n_days] effective_reproduction_number;
  vector[n_days] twitter_symptoms_lagged_daily_infections;
  vector[n_days] daily_twitter_symptoms;
  ...
  // Populate States
  state_S = append_row(y0[1] + machine_precision(), to_vector(state_estimate[, 1]));
  state_I = append_row(y0[2] + machine_precision(), to_vector(state_estimate[, 2]));
  state_R = append_row(y0[3] + machine_precision(), to_vector(state_estimate[, 3]));
  state_D = append_row(y0[4] + machine_precision(), to_vector(state_estimate[, 4]));

  // Transform to Daily stuff
  daily_infections = state_S[:n_days] - state_S[2:];
  daily_deaths = state_D[2:] - state_D[:n_days];
  effective_reproduction_number= (daily_infections ./ state_I[:n_days]) / gamma;
