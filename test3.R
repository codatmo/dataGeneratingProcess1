...
model <- cmdstan_model("stan/tweet_sird_negbin_deaths.stan")
...
stan_data <- list(n_days = nrow(simDf), y0 = c(nPop - infStartValue, infStartValue, 0, 0), t0 = 0,
                  ts = 1:nDays, N = nPop, death_count = simDf$d,
...
resultsBetaDf <- fit$summary(variables = "beta")
resultsGammaDf <- fit$summary(variables = "gamma")
resultsDeathDf <- fit$summary(variables = "deaths")
