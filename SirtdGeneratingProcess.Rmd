...
stan_data <- list(
  n_days = nrow(simDf),
  y0 = c(nPop - first(simDf$i), first(simDf$i), first(simDf$r), first(simDf$d)),
  t0 = 0,
  ts = 1:nDays,
  N = nPop,
  death_count = simDf$d,
...
```{r}
stan_variables <- c("beta", "gamma", "omega", "phi_inv", "recovery_time", "R0")
...
```{r, dpi=300}
daily_infected <- fit$summary("daily_infections")$mean
